...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <motion.div 
      className="group relative bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-white/50 hover:border-purple-200 transform-gpu"
    ...
    >
      ...
      <div className="absolute top-3 left-3 z-20 flex flex-col gap-1.5">
        {discountPercent > 0 && (
          <motion.div 
            ...
            className="bg-gradient-to-r from-red-500 to-pink-500 text-white px-2 py-1 text-xs font-semibold rounded-lg shadow-md flex items-center gap-1"
          >
            <Zap size={10} />
            -{discountPercent}%
          </motion.div>
        ...
        {product.featured && (
          <motion.div 
            ...
            className="bg-gradient-to-r from-orange-500 to-red-500 text-white px-2 py-1 text-xs font-semibold rounded-lg shadow-md flex items-center gap-1"
          >
            <Award size={10} />
            HOT
          </motion.div>
      ...
      <div className="p-4 space-y-3">
        <Link href={`/products/${product.id}`}>
          <h3 className="font-semibold text-gray-900 mb-1.5 hover:text-purple-600 transition-colors line-clamp-2 text-sm min-h-[2.5rem] leading-5">
            {product.name}
          </h3>
        </Link>
        ...
        {availableColors.length > 0 && (
          <div className="flex items-center gap-1.5">
            <span className="text-xs text-gray-500 font-medium">Màu:</span>
        ...
        <motion.button 
          ...
          className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-2.5 px-3 rounded-xl transition-all flex items-center justify-center gap-2 font-semibold text-sm shadow-md hover:shadow-lg group/btn"
      ...
      <AnimatePresence mode="wait">
        {showQuickAdd && (
          ...
          >
            <motion.div 
              className="relative bg-white rounded-2xl max-w-sm w-full my-4 shadow-xl border border-gray-200 z-[10000]"
              initial={{ 
                scale: 0.8, 
                opacity: 0, 
                y: 30,
            ...
            >
              ...
              <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-gradient-to-r from-purple-50 to-pink-50">
                ...
                <motion.button
                  onClick={() => setShowQuickAdd(false)}
                  className="p-1.5 hover:bg-white/80 rounded-full transition-all"
              ...
              <div className="p-4 space-y-4">
                {/* Product Info */}
                <motion.div 
                  className="flex gap-3 bg-gradient-to-r from-gray-50 to-gray-100 p-3 rounded-xl"
                ...
                >
                  <div className="w-16 h-16 flex-shrink-0 rounded-lg overflow-hidden border border-white shadow-md">
                    <Image
                      src={productImage}
                      alt={product.name || 'Product'}
                      width={64}
                  ...
                  <div className="flex-1">
                    <h4 className="font-semibold text-sm text-gray-900 mb-1 line-clamp-2">{product.name || 'Sản phẩm'}</h4>
                    <p className="font-bold text-base text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600">
                ...
                >
                  <h4 className="font-semibold text-sm text-gray-900 mb-3 flex items-center gap-1.5">
                ...
                >
                  <motion.button
                    onClick={() => setShowQuickAdd(false)}
                    className="flex-1 px-4 py-2.5 border-2 border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-all font-semibold text-sm"
                  ...
                  <motion.button
                    ...
                    className="flex-2 flex-grow px-4 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:shadow-lg transition-all font-semibold text-sm flex items-center justify-center gap-2"
                    whileHover={{ scale: 1.02, y: -1 }}
                    whileTap={{ scale: 0.98 }}
                  >
